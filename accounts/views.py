...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_email.send()
            # messages.success(
            #     request, 'An email has been sent with instructions to activate your account')
...
def order_detail(request, order_id):
    ...
    order_detail_first = OrderProduct.objects.filter(
        order__order_number=order_id).first()
    subtotal = 0
